...
class Anagram:
    ...
    def match_anagrams(self, input_word):  # returns list of matching anagrams
        ...
        #  iterating through each word in dictionary and comparing with input word
        for dict_word, dict_word_encoding in self.input_dictionary_encoding.items():

            #  Check for words length matching before comparing their encodings as words should be of equal length
            if input_word_len == len(dict_word):

                #  Comparing the words encodings
                bool_matching = self.compare_encodings(input_word_encoding, dict_word_encoding)
                if bool_matching:
                    #  if condition is True ( they match) add to the result list
                    result_matching_anagrams.append(dict_word)
